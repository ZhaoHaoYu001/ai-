...
- **轮次式语音对话**：采用成熟的显式半双工轮次交互；点击开始回答后持续累积本轮语音与文字，停顿或识别服务重连不会自动发送，点击结束后才统一提交给 AI。
...
1. 从首页进入任意场景。
2. 使用“填入纠错示例”快速触发对话和纠错；开始语音回答后验证回答支架始终可见。
...
3. 在 Chrome / Edge 点击“开始语音回答”，说完后点击“结束回答并发送”，验证本轮转写、语音播放控制和清晰度代理。
