...
> Demo 视频：[七牛云暑期实训营作品：AI 英语口语陪练（哔哩哔哩）](https://b23.tv/VW4DosC)
...
在中国大陆推荐使用腾讯云智聆口语评测，支持英文单词与音素级反馈：

```powershell
Copy-Item .env.example .env
# 在 .env 中填写 TENCENT_SPEECH_APP_ID、TENCENT_SPEECH_SECRET_ID 和 TENCENT_SPEECH_SECRET_KEY
npm.cmd start
```

也可使用 Azure Speech 启用真实单词、音素、重音与韵律评测：
...
详细接口、隐私与降级策略见 `docs/AI_COACH.md`、`docs/TENCENT_PRONUNCIATION.md` 和 `docs/AZURE_PRONUNCIATION.md`。
...
- **自主节奏回答**：点击“开始回答”后可自由停顿和组织语言，确认表达完整后点击“提交回答”才进入评测，避免系统在思考停顿时提前结束。
- **自适应轮次结束**：已有至少 8 个实时转写单词且持续表达超过 4 秒时，静音结束窗口会从 2.4 秒缩短到约 1.8 秒；短回答或无转写时继续使用更稳妥的 2.4 秒窗口。
- **可控口语训练模式**：Coach 新问题生成后只显示文本，不会立即朗读或抢先开启下一轮录音；练习者可手动播放问题、开始回答、提交回答，并在需要时结束练习生成报告。
...
- **专业发音评测**：优先支持中国大陆可用的腾讯云智聆口语评测，也可配置 Azure Speech；展示准确度、流利度、完整度、具体单词和音素问题，未配置时明确降级为浏览器清晰度代理。
...
3. 在 Chrome / Edge 进入场景，播放 AI 问题后点击“开始回答”，验证自由停顿、手动提交回答、连续追问和清晰度代理。
...
5. 从无登录状态打开 README 顶部的 Demo 视频链接并完整播放一次，确认评委可正常访问。
