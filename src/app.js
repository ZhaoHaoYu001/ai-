...
const state = { view: "home", scenario: null, messages: [], elapsed: 0, timer: null, listening: false, recognition: null, liveVoice: false, interim: "", coachSpeaking: false, coachPaused: false, audioCapture: null, sessionAudioUrl: null, assessmentClient: createSpeechAssessmentClient(window.FLUENTLOOP_SPEECH_SERVICE || {}) };
...
`<article class="msg user"><div><small>YOU</small><p>${m.text}</p><section class="feedback"><strong>${m.analysis.scores.overall}</strong><span><b>${m.analysis.corrections.length?"表达清晰，留意下面的小调整。":"表达自然且清晰，句型使用得很好。"}</b><small>${m.analysis.wpm} WPM · ${m.analysis.words} words</small></span>${m.analysis.speechEvidence ? `<div class="evidence"><b>${m.analysis.speechEvidence.level==="phoneme"?"音素级评测":"浏览器清晰度代理"} · ${m.analysis.scores.pronunciation}</b><small>${m.analysis.speechEvidence.disclaimer}</small></div>` : `<div class="evidence muted"><b>未进行语音评测</b><small>文字输入不会生成发音或语音清晰度分数。</small></div>`}${m.analysis.corrections.map(c=>`<article><label>表达建议</label><del>${c.original}</del><b>→ ${c.improved}</b><small>${c.reason}</small></article>`).join("")||"<em>✓ 本轮没有发现明显语法问题</em>"}</section></div><i>YOU</i></article>`;
