...
const state = { view: "home", scenario: null, sessionId: 0, messages: [], answerQueue: [], processingAnswer: false, elapsed: 0, timer: null, listening: false, recognition: null, speechStartedAt: null, liveVoice: false, interim: "", coachSpeaking: false, coachPaused: false, audioCapture: null, sessionAudioUrl: null, aiBusy: false, aiMode: "checking", assessmentClient: createSpeechAssessmentClient(window.FLUENTLOOP_SPEECH_SERVICE || { endpoint: "/api/pronunciation" }), aiCoach: createAiCoachClient() };
...
async function start(id){ state.sessionId++;const sessionId=state.sessionId;state.answerQueue=[];state.scenario=scenarios.find(s=>s.id===id); state.messages=[{role:"coach",text:state.scenario.opening,translation:translateCoachText(state.scenario.opening,id)}]; state.view="practice"; state.liveVoice=true; state.interim=""; state.sessionAudioUrl=null; state.elapsed=0;clearInterval(state.timer);state.timer=setInterval(()=>{state.elapsed++;const t=document.querySelector("#timer");if(t)t.textContent=format(state.elapsed)},1000);try{state.audioCapture=await createAudioCapture()}catch{state.audioCapture=null;state.liveVoice=false}practice();setTimeout(()=>{if(state.view==="practice"&&state.sessionId===sessionId)speak(state.scenario.opening)},300)}
...
async function addVoiceMessage(text,confidence,seconds){const startedAt=performance.now(),assessmentStartedAt=performance.now(),audioBlob=state.audioCapture?.utteranceBlob()||null;const evidence=await state.assessmentClient.assess(audioBlob,text,{confidence,...(state.audioCapture?.snapshot()||{})});state.interim="";state.speechStartedAt=null;await handleAnswer(text,seconds,evidence,{startedAt,assessmentMs:Math.round(performance.now()-assessmentStartedAt)})}
